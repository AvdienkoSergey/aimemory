...
let cleanup paths = List.iter (fun p -> try Sys.remove p with _ -> ()) paths
...
let test_info_writes_to_file _ =
  ...
  assert_bool "contains message"
    (try
       let _ = Str.search_forward (Str.regexp "test message 123") content 0 in
       true
...
let test_debug_respects_level _ =
  ...
  assert_bool "no debug message"
    (not
       (try
          let _ =
            Str.search_forward (Str.regexp "debug_should_not_appear") content 0
          in
          true
        with Not_found -> false));
  assert_bool "has info message"
    (try
       let _ = Str.search_forward (Str.regexp "info_should_appear") content 0 in
       true
     with Not_found -> false);
  cleanup [ log_path ]
...
let test_verbose_includes_debug _ =
  ...
  assert_bool "has debug message"
    (try
       let _ = Str.search_forward (Str.regexp "debug_visible_now") content 0 in
       true
     with Not_found -> false);
  cleanup [ log_path ]
...
let test_log_format_has_timestamp _ =
  ...
  assert_bool "has timestamp"
    (try
       let _ = Str.search_forward timestamp_regex content 0 in
       true
     with Not_found -> false);
  cleanup [ log_path ]
...
let test_log_format_has_level _ =
  ...
  assert_bool "has WARN level"
    (try
       let _ = Str.search_forward (Str.regexp {|\[WARN|}) content 0 in
       true
     with Not_found -> false);
  cleanup [ log_path ]
