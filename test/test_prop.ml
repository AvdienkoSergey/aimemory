...
let gen_lid =
  Gen.map2 (fun k p -> Lid.make k ~path:p) gen_kind gen_path

let gen_rel =
  Gen.oneof_list [
    Ref.Belongs_to; Ref.Calls; Ref.Depends_on;
    Ref.Contains; Ref.Implements; Ref.References
  ]

let rec gen_value depth =
  if depth <= 0 then gen_leaf_value
  else Gen.oneof [
    gen_leaf_value;
    Gen.map (fun vs -> Entity.List vs)
      (Gen.list_size (Gen.int_range 0 3) (gen_value (depth - 1)));
  ]

and gen_leaf_value =
  Gen.oneof [
    Gen.map (fun s -> Entity.String s) (Gen.string_size ~gen:Gen.printable (Gen.int_range 0 20));
...
let gen_key =
  Gen.string_size ~gen:gen_alphanum (Gen.int_range 1 10)
...
let arb_printable_string = make ~print:(Printf.sprintf "%S")
  (Gen.string_size ~gen:Gen.printable (Gen.int_range 0 50))

(* ── Properties ── *)

let prop_lid_roundtrip =
  Test.make ~name:"Lid roundtrip: of_string . to_string = id" ~count:500
    arb_lid
    (fun lid ->
...
let prop_lid_format =
  Test.make ~name:"Lid to_string contains colon" ~count:500
    arb_lid
    (fun lid -> String.contains (Lid.to_string lid) ':')

let prop_lid_kind_preserved =
  Test.make ~name:"Lid kind preserved through roundtrip" ~count:500
    arb_lid
    (fun lid ->
       match Lid.of_string (Lid.to_string lid) with
...
let prop_lid_path_preserved =
  Test.make ~name:"Lid path preserved through roundtrip" ~count:500
    arb_lid
    (fun lid ->
       match Lid.of_string (Lid.to_string lid) with
...
let prop_rel_roundtrip =
  Test.make ~name:"Ref.rel roundtrip" ~count:100
    arb_rel
    (fun rel -> Ref.rel_of_string (Ref.rel_to_string rel) = Some rel)

let prop_data_json_roundtrip =
  Test.make ~name:"Entity.data JSON roundtrip" ~count:500
    (make gen_data)
    (fun data ->
...
let prop_string_value_roundtrip =
  Test.make ~name:"Entity.String roundtrip" ~count:500
...
let prop_int_value_roundtrip =
  Test.make ~name:"Entity.Int roundtrip" ~count:500
    int
    (fun i ->
...
let prop_bool_value_roundtrip =
  Test.make ~name:"Entity.Bool roundtrip" ~count:100
    bool
    (fun b ->
...
let prop_all_kinds_have_prefix =
  Test.make ~name:"All kinds have non-empty prefix" ~count:100
    arb_kind
...
let () =
  let suite = List.map QCheck_ounit.to_ounit2_test [
    prop_lid_roundtrip;
    prop_lid_format;
