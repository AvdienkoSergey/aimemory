...
let test_parse_emit_valid _ctx =
  ...
  | Ok (Protocol.Emit { entities }) -> assert_equal 1 (List.length entities)
...
let test_parse_query_entities_with_pattern _ctx =
  ...
  match Tools.parse_query_entities json with
  | Ok (Protocol.Query_entities q) -> assert_equal (Some "auth/*") q.pattern
...
let suite =
  ...
  >::: [
         ...
         "parse_entity_input_missing_lid"
         >:: test_parse_entity_input_missing_lid;
         "parse_entity_input_invalid_lid"
         >:: test_parse_entity_input_invalid_lid;
         ...
         "parse_query_entities_pattern"
         >:: test_parse_query_entities_with_pattern;
