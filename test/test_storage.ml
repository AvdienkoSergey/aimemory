...
let test_pp_error_not_found _ctx =
  let lid =
    match Lid.of_string "issue:foo" with
    | Ok l -> l
...
let test_upsert_insert _ctx =
  ...
  let raw = make_raw "issue:greet" ~data:[ ("name", Entity.String "Alice") ] () in
...
let test_find_by_lid_existing _ctx =
  let db = open_mem _ctx in
  let raw = make_raw "issue:hello" () in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_find_by_lid_missing _ctx =
  let db = open_mem _ctx in
  let lid =
    match Lid.of_string "issue:ghost" with
    | Ok l -> l
    | Error _ -> assert_failure "bad lid"
  in
...
let test_upsert_many _ctx =
  let db = open_mem _ctx in
  let raws = [ make_raw "issue:a" (); make_raw "issue:b" (); make_raw "issue:c" () ] in
...
let test_query_entities_all _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.upsert_many db [ make_raw "issue:x" (); make_raw "mr:x" () ]));
...
let test_query_entities_by_kind _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.upsert_many db
          [ make_raw "issue:foo" (); make_raw "issue:bar" (); make_raw "mr:baz" () ]));
  let kind =
    match Lid.of_string "issue:dummy" with
    | Ok l -> Lid.kind l
    | Error _ -> assert_failure "bad lid"
  in
...
let test_query_entities_by_pattern _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.upsert_many db
          [
            make_raw "issue:auth/login" ();
            make_raw "issue:auth/logout" ();
            make_raw "issue:home" ();
          ]));
  let results, _total =
...
let test_delete_missing _ctx =
  let db = open_mem _ctx in
  let lid =
    match Lid.of_string "issue:nobody" with
    | Ok l -> l
    | Error _ -> assert_failure "bad lid"
  in
...
let test_data_roundtrip_string _ctx =
  let db = open_mem _ctx in
  let raw =
    make_raw "issue:rt" ~data:[ ("greeting", Entity.String {|He said "hi"|}) ] ()
  in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_data_roundtrip_int _ctx =
  let db = open_mem _ctx in
  let raw = make_raw "issue:rt_int" ~data:[ ("count", Entity.Int 42) ] () in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_data_roundtrip_float _ctx =
  let db = open_mem _ctx in
  let raw = make_raw "issue:rt_float" ~data:[ ("pi", Entity.Float 3.14) ] () in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_data_roundtrip_null _ctx =
  let db = open_mem _ctx in
  let raw = make_raw "issue:rt_null" ~data:[ ("nothing", Entity.Null) ] () in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_data_roundtrip_list _ctx =
  ...
  let raw = make_raw "issue:rt_list" ~data:[ ("nums", lst) ] () in
  let stored = ok_or_fail (Repo.upsert db raw) in
  assert_equal raw.Entity.data stored.Entity.data;
  Repo.close db
...
let test_data_empty _ctx =
  let db = open_mem _ctx in
  let raw = make_raw "issue:empty" ~data:[] () in
  let stored = ok_or_fail (Repo.upsert db raw) in
...
let test_insert_ref_pending _ctx =
  let db = open_mem _ctx in
  let p = make_pending "issue:a" "issue:b" Ref.Linked_to in
...
let test_insert_refs_batch _ctx =
  ...
  let refs =
    [
      make_pending "issue:a" "issue:b" Ref.Linked_to;
      make_pending "issue:b" "issue:c" Ref.References;
    ]
  in
...
let test_resolve_all_both_exist _ctx =
  let db = open_mem _ctx in
  ignore (ok_or_fail (Repo.upsert db (make_raw "issue:a" ())));
  ignore (ok_or_fail (Repo.upsert db (make_raw "issue:b" ())));
  ignore
    (ok_or_fail (Repo.insert_ref db (make_pending "issue:a" "issue:b" Ref.Linked_to)));
...
let test_resolve_all_mixed _ctx =
  let db = open_mem _ctx in
  ignore (ok_or_fail (Repo.upsert db (make_raw "issue:x" ())));
  ignore (ok_or_fail (Repo.upsert db (make_raw "issue:y" ())));
  ignore
    (ok_or_fail (Repo.insert_ref db (make_pending "issue:x" "issue:y" Ref.Linked_to)));
  ignore
    (ok_or_fail
       (Repo.insert_ref db (make_pending "issue:x" "issue:z" Ref.References)));
  let resolved, pending = ok_or_fail (Repo.resolve_all db) in
  assert_equal 1 (List.length resolved);
  assert_equal 1 (List.length pending);
  Repo.close db
...
let setup_resolved_refs db =
  let la =
    match Lid.of_string "issue:a" with
    | Ok l -> l
    | Error _ -> assert_failure "bad"
  in
  let lb =
    match Lid.of_string "issue:b" with
    | Ok l -> l
    | Error _ -> assert_failure "bad"
  in
  let lc =
    match Lid.of_string "issue:c" with
    | Ok l -> l
    | Error _ -> assert_failure "bad"
  in
  ignore
    (ok_or_fail
       (Repo.upsert_many db
          [ make_raw "issue:a" (); make_raw "issue:b" (); make_raw "issue:c" () ]));
  ignore
    (ok_or_fail
       (Repo.insert_refs db
          [
            make_pending "issue:a" "issue:b" Ref.Linked_to;
            make_pending "issue:b" "issue:c" Ref.References;
            make_pending "issue:a" "issue:c" Ref.Linked_to;
          ]));
...
let test_query_refs_by_rel _ctx =
  let db = open_mem _ctx in
  ignore (setup_resolved_refs db);
  let refs, _total = ok_or_fail (Repo.query_refs db ~rel:Ref.Linked_to ()) in
  assert_equal 2 (List.length refs);
  List.iter (fun r -> assert_equal Ref.Linked_to r.Ref.rel) refs;
  Repo.close db
...
let test_delete_cascades_refs _ctx =
  ...
  ignore
    (ok_or_fail (Repo.insert_ref db (make_pending "issue:a" "issue:b" Ref.Linked_to)));
  let lid_a =
    match Lid.of_string "issue:a" with Ok l -> l | Error _ -> assert_failure ""
  in
...
let test_stats_after_ops _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.upsert_many db [ make_raw "issue:a" (); make_raw "issue:b" () ]));
  ignore
    (ok_or_fail (Repo.insert_ref db (make_pending "issue:a" "issue:b" Ref.Linked_to)));
  ignore
    (ok_or_fail
       (Repo.insert_ref db (make_pending "issue:a" "issue:z" Ref.References)));
  ignore (ok_or_fail (Repo.resolve_all db));
  let s = ok_or_fail (Repo.stats db) in
...
let test_all_lids _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.upsert_many db
          [ make_raw "issue:a" (); make_raw "issue:b" (); make_raw "mr:x" () ]));
...
let test_with_tx_commit _ctx =
  ...
  ignore
    (ok_or_fail
       (Repo.with_tx db (fun db -> Repo.upsert db (make_raw "issue:tx" ()))));
  (match
     ok_or_fail
       (Repo.find_by_lid db
          (match Lid.of_string "issue:tx" with
          | Ok l -> l
...
let test_with_tx_rollback _ctx =
  let db = open_mem _ctx in
  let lid =
    match Lid.of_string "issue:rollback" with
    | Ok l -> l
    | Error _ -> assert_failure ""
  in
  let _ =
    Repo.with_tx db (fun db ->
        let _ = Repo.upsert db (make_raw "issue:rollback" ()) in
