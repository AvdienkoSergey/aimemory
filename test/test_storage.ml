...
let test_query_entities_all _ctx =
  ...
  let all = ok_or_fail (Repo.query_entities db ()) in
...
let test_query_entities_by_kind _ctx =
  ...
  let kind =
    ...
    | Error _ -> assert_failure "bad lid"
  in
  let results = ok_or_fail (Repo.query_entities db ~kind ()) in
...
let test_query_entities_by_pattern _ctx =
  ...
  let results = ok_or_fail (Repo.query_entities db ~pattern:"auth/%" ()) in
  assert_equal 2 (List.length results);
  Repo.close db
...
let test_query_refs_all _ctx =
  ...
  let refs = ok_or_fail (Repo.query_refs db ()) in
...
let test_query_refs_by_source _ctx =
  ...
  let refs = ok_or_fail (Repo.query_refs db ~source:la ()) in
...
let test_query_refs_by_target _ctx =
  ...
  let refs = ok_or_fail (Repo.query_refs db ~target:lc ()) in
  assert_equal 2 (List.length refs);
...
let test_query_refs_by_rel _ctx =
  let db = open_mem _ctx in
  ignore (setup_resolved_refs db);
  let refs = ok_or_fail (Repo.query_refs db ~rel:Ref.Calls ()) in
  assert_equal 2 (List.length refs);
...
let test_query_refs_combined _ctx =
  ...
  let refs = ok_or_fail (Repo.query_refs db ~source:la ~target:lc ()) in
