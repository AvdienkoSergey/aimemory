(** Tools — MCP-like tool interface for AI interaction.
    ...
    All parsing errors become structured JSON error responses —
    the AI never sees OCaml exceptions. *)
...
let rel_of_json (j : t) : Ref.rel option =
  match j with
  | `String s -> Ref.rel_of_string s
  | _ -> None

let rel_to_json (r : Ref.rel) : t =
  `String (Ref.rel_to_string r)
...
let response_to_json (resp : Protocol.response) : t =
  ...
  | Emit_result r ->
    `Assoc [
      ("status", `String "ok");
      ("command", `String "emit");
      ("upserted", `List (List.map (fun lid ->
         `String (Lid.to_string lid)) r.upserted));
...
(** Parse a single entity_input from JSON.
    Expected shape:
    {[ { "lid": "mod:auth/login",
...
let parse_entity_input (j : t) : (Protocol.entity_input, string) result =
  match j with
  | `Assoc fields ->
    let lid_s = match List.assoc_opt "lid" fields with
      | Some (`String s) -> Some s | _ -> None in
    (match lid_s with
     | None -> Error "entity_input: missing or invalid 'lid' field"
     | Some s ->
       match Lid.of_string s with
       | Error e -> Error (Printf.sprintf "Invalid LID '%s': %s" s (Lid.pp_parse_error e))
       | Ok lid ->
         let data = match List.assoc_opt "data" fields with
           | Some d -> json_to_data d
           | None -> []
         in
         let refs = match List.assoc_opt "refs" fields with
           | Some (`List rs) ->
             List.filter_map (fun r ->
               match r with
               | `Assoc rf ->
                 let target_s = match List.assoc_opt "target" rf with
                   | Some (`String s) -> Some s | _ -> None in
                 let rel_s = match List.assoc_opt "rel" rf with
                   | Some (`String s) -> Some s | _ -> None in
                 (match target_s, rel_s with
                  | Some ts, Some rs ->
                    (match Lid.of_string ts, Ref.rel_of_string rs with
                     | Ok target, Some rel ->
...
let parse_emit (j : t) : (Protocol.command, string) result =
  match j with
  | `Assoc fields ->
    let entities_json = match List.assoc_opt "entities" fields with
      | Some (`List es) -> es
      | _ -> []
    in
...
let parse_query_entities (j : t) : (Protocol.command, string) result =
  ...
  | `Assoc fields ->
    let kind = match List.assoc_opt "kind" fields with
      ...
      | None -> None
    in
    let pattern = match List.assoc_opt "pattern" fields with
  ...
  | `Assoc fields ->
    let parse_lid_opt key =
      ...
      | Some (`String s) ->
        (match Lid.of_string s with
         | Ok lid -> Ok (Some lid)
         | Error e -> Error (Printf.sprintf "Invalid LID in '%s': %s"
                               key (Lid.pp_parse_error e)))
      | _ -> Ok None
    in
    (match parse_lid_opt "source", parse_lid_opt "target" with
     | Ok source, Ok target ->
       let rel_type = match List.assoc_opt "rel" fields with
    ...
    if tool = "status" then
      ...
      | Ok s ->
        Yojson.Safe.to_string (`Assoc [
          ...
          ("missing_entities", `List (List.map (fun (lid, n) ->
             `Assoc [
...
(** Generate MCP-compatible tool definitions as JSON.
    Inject this into the AI's system prompt so it knows
  ...
  `Assoc [
    ...
    ("tools", `List [
      ...
      `Assoc [
        ("name", `String "emit");
        ("description", `String
           "Upsert entities into context memory with optional references. \
            Entities are identified by LID (logical ID) in format 'kind:path'. \
            References to non-existent entities are stored as pending and \
            auto-resolved when the target appears.");
        ("parameters", `Assoc [
          ("type", `String "object");
          ...
          ("properties", `Assoc [
            ("entities", `Assoc [
              ("type", `String "array");
              ("description", `String "Array of entities to upsert");
              ("items", `Assoc [
                ("type", `String "object");
                ...
                ("properties", `Assoc [
                  ...
                  ("data", `Assoc [
                    ("type", `String "object");
                    ("description", `String
                       "Arbitrary key-value data for this entity. \
                        Values can be strings, numbers, booleans, arrays, or null.");
                  ]);
                  ("refs", `Assoc [
                    ("type", `String "array");
                    ("description", `String "References to other entities");
                    ("items", `Assoc [
                      ("type", `String "object");
                      ("required", `List [`String "target"; `String "rel"]);
                      ("properties", `Assoc [
                        ("target", `Assoc [
                          ("type", `String "string");
                          ("description", `String "LID of the target entity");
                        ]);
                        ("rel", `Assoc [
                          ("type", `String "string");
      ...
      `Assoc [
        ...
        ("description", `String
           "Query entities in context memory. \
            Use to discover what LIDs already exist before emitting refs. \
            Returns entities with their data and physical IDs.");
        ("parameters", `Assoc [
          ("type", `String "object");
          ("properties", `Assoc [
            ("kind", `Assoc [
      ...
      `Assoc [
        ...
        ("description", `String
           "Query resolved references between entities. \
            Only returns refs where both source and target exist.");
        ("parameters", `Assoc [
          ("type", `String "object");
          ("properties", `Assoc [
            ("source", `Assoc [
      ...
      `Assoc [
        ("name", `String "status");
        ("description", `String
           "Get diagnostics: entity count, resolved/pending ref counts, \
            and list of missing entities blocking resolution. \
