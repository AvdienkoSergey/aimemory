(** MCP Server — JSON-RPC 2.0 stdio transport for Model Context Protocol.
    ...
    Reads newline-delimited JSON from stdin, dispatches to Tools, writes JSON
    responses to stdout. All logging goes to the log file, never to stdout
      ...
      | Some (`List tools) ->
          List.map
            (fun tool ->
              ...
              | `Assoc tfields ->
                  let name =
                    List.assoc_opt "name" tfields
                    |> Option.value ~default:(`String "")
                  in
                  let description =
                    List.assoc_opt "description" tfields
                    |> Option.value ~default:(`String "")
                  in
                  let input_schema =
                    match List.assoc_opt "parameters" tfields with
                    | Some params -> params
...
let handle_request db (msg : t) : t option =
  ...
  let method_name =
    match List.assoc_opt "method" fields with Some (`String s) -> s | _ -> ""
  in
  let params =
    match List.assoc_opt "params" fields with Some p -> p | None -> `Assoc []
  in
           ...
           (`Assoc
              [
                ...
                ( "serverInfo",
                  `Assoc
                    [
                      ("name", `String "aimemory"); ("version", `String "0.1.1");
                    ] );
              ]))
  | "notifications/initialized" -> None
  | "tools/list" ->
      Some
        (jsonrpc_response ~id (`Assoc [ ("tools", `List (mcp_tools_list ())) ]))
  | "tools/call" ->
      let pfields = match params with `Assoc f -> f | _ -> [] in
      let tool_name =
        match List.assoc_opt "name" pfields with
        | Some (`String s) -> s
        | _ -> ""
      in
      let tool_args =
        match List.assoc_opt "arguments" pfields with
              ...
              [
                ( "content",
                  `List
                    [
                      `Assoc
                        [ ("type", `String "text"); ("text", `String result) ];
                    ] );
              ]))
  | "ping" -> Some (jsonrpc_response ~id (`Assoc []))
  | _ ->
      Some
    ...
    while true do
      ...
      if String.trim line <> "" then begin
        ...
        | exception Yojson.Json_error e ->
            send
              (jsonrpc_error ~id:`Null ~code:(-32700)
