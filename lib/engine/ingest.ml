(** Ingest — command processor and pipeline orchestrator.
    ...
    This is the single entry point for the engine layer.
    Takes [Protocol.command], executes against [Repo.t],
    returns [Protocol.response].

    Pipeline for [Emit]:
    1. Extract entities and refs from [entity_input] list
    2. Upsert all entities in a transaction
    3. Insert all pending refs
    4. Run resolver to link what's possible
    5. Return [emit_result] with resolved/pending split
...
let process_emit (db : Repo.t) (inputs : Protocol.entity_input list)
  ...
  let raws : Entity.raw list =
    List.map (fun (ei : Protocol.entity_input) ->
      Entity.{ lid = ei.lid; data = ei.data }
    ) inputs
  in
  (* Step 2: extract pending refs from each input *)
  let all_pending : Ref.pending list =
    List.concat_map (fun (ei : Protocol.entity_input) ->
      List.map (fun (ri : Protocol.ref_input) ->
  ...
  | Ok stored ->
    ...
    | Ok () ->
      ...
      | Error e -> Protocol.Error (Storage_error (Repo.pp_error e))
      | Ok resolution ->
        let upserted = List.map (fun (s : Entity.processed) -> s.lid) stored in
...
(** Process multiple commands sequentially.
    Stops on first error, returns all responses up to that point. *)
...
(** Process multiple commands, collecting all responses
...
let emit_one (db : Repo.t) ~lid ~data
    : Protocol.response =
  let input = Protocol.{
    lid;
    data;
...
let emit_with_refs (db : Repo.t) ~lid ~data ~refs
    : Protocol.response =
  let ref_inputs = List.map (fun (target, rel) ->
    Protocol.{ target; rel }
  ) refs in
  let input = Protocol.{
    lid;
...
type status = {
  stats   : Repo.stats;
  missing : Resolver.missing_analysis;
}
...
let pp_status (s : status) : string =
  Printf.sprintf
    "Entities: %d | Refs resolved: %d | Refs pending: %d\n%s"
    s.stats.entity_count
    s.stats.ref_resolved_count
    s.stats.ref_pending_count
